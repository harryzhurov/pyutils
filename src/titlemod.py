...
def process_options(options):
    ...
    optlist, infiles = getopt.gnu_getopt(options, 'rd:v:')
...
#
def modify_title(f, vernum, year):
    ...
    for i in in_file:
        ...
        if res:
            Copyright = res.group(0)
            yres = re.search('([0-9]+).*', Copyright)
            if yres:
                y = yres.group(1)
                if y != year:
                    newdate = y + '-' + year
                    in_file[idx] = i.replace(yres.group(0), newdate, 1)
